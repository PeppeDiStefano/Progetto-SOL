...
typedef struct threadpool_t {
    ...
    pthread_cond_t   condFull;// usata per notificare un thread in attesa sulla add
 ...
 * @brief aggiunge un task al pool, se ci sono thread liberi il task viene assegnato ad uno di questi, se non ci sono thread liberi e pending_size > 0 allora si cerca di inserire il task come task pendente. Se non c'e' posto nella coda interna allora si rimane in attesa che si liberi un posto
 * @param pool oggetto thread pool
 * @param fun  funzione da eseguire per eseguire il task
 * @param arg  argomento della funzione
 * @return 0 se successo, 1 se devo terminare o non si possono gestire richieste pendeneti, -1 in caso di fallimento, errno viene settato opportunamente.
 */
